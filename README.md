...
本版本完全遵循[MoviePilot官方插件开发规范](https://github.com/jxxghp/MoviePilot-Plugins)，参考官方CustomIndexer插件实现模式：

- ✅ **权限级别**：`auth_level = 2`，允许所有完成MoviePilot用户认证的用户使用插件
- ✅ **注册方式**：直接使用`SitesHelper().add_indexer()`，自动覆盖已有索引器
- ✅ **代码简洁**：移除不必要的检查和深拷贝，代码更清晰易维护
- ✅ **用户认证**：MoviePilot系统层面处理用户认证，插件无需关心

### 官方文档说明

根据官方文档"如何扩展内建索引器的索引站点"：

> "需要注意的是，如果你没有完成用户认证，通过插件配置进去的索引站点也是无法正常使用的。"

**理解**：
- 这是给**插件使用者**的提示，不是给插件开发者的要求
- MoviePilot会在**用户层面**检查认证状态
- 插件只需正确注册索引器到站点管理即可
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 1.0.0 | 2026-02-13 | 📚 符合官方规范的正式版本：auth_level改为2（允许所有认证用户使用）；简化索引器注册逻辑，遵循官方CustomIndexer模式；移除不必要的检查和深拷贝；说明MoviePilot用户认证由系统处理，插件无需检查 |
