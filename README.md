...
5. **添加站点**：
   - 查看插件数据页面获取索引器列表
   - 在 MoviePilot 站点管理中添加对应站点
   - 站点地址格式：`https://prowlarr.indexer.<indexer_id>` 或 `https://jackett.indexer.<indexer_id>`
6. **开始搜索**：在 MoviePilot 中搜索资源时会自动调用已添加的索引器
...
这是正常现象。由于插件使用虚拟域名注册索引器，MoviePilot 的站点连通性检测会显示异常，但不影响搜索功能。
