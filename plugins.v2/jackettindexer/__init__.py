...
class JackettIndexer(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        # Fetch indexers from Jackett first
        logger.info(f"【{self.plugin_name}】开始从Jackett获取索引器...")
    ...
    def _build_indexer_dict(self, indexer: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Build domain identifier using indexer title for readability
        # Convert to lowercase and replace spaces/special chars with hyphens
        indexer_slug = re.sub(r'[^a-z0-9]+', '-', indexer_title.lower()).strip('-')
