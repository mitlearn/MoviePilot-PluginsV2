...
**说明**:
- 触发Trakt想看列表同步（Watchlist + 自定义列表）
- 根据"添加启用的订阅"配置决定新订阅的初始状态（N=激活 / S=暂停）
- 已在订阅列表中的媒体（含暂停状态）会被跳过，不会修改其状态
- 异步执行，立即返回
...
**说明**:
- 仅触发自定义列表同步，不处理 Watchlist
- 同步配置中所有列表的电影和剧集（不受 Watchlist 同步类型限制）
- 根据"添加启用的订阅"配置决定新订阅的初始状态
- 异步执行，立即返回
...
| 1.5 | 2026-02-17 | 修复订阅检测逻辑：改用 `SubscribeOper().exists()` 覆盖暂停状态；修复单季季号检测精度；`add_and_enable` 仅影响新增订阅行为说明 |
